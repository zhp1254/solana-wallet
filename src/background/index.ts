...
chrome.runtime.onConnect.addListener((port) => {
    ...
    if (port.name === CONTENT_PORT_NAME) {
        ...
        rpc.exposeMethod('signMsg', async (data) => {
            const { closePopup, popupClosed } = await openPopup('signMsg');

            const popupRPC = await asyncPopupRPC.get();
            const connectResult = popupRPC.callMethod('signMsg', data);
        ...
        rpc.exposeMethod('signTx', async (data) => {
            const { closePopup, popupClosed } = await openPopup('signTx');

            const popupRPC = await asyncPopupRPC.get();
            const connectResult = popupRPC.callMethod('signTx', data);
