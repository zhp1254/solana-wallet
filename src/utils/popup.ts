...
async function createPopup(params): Promise<chrome.windows.Window> {
    const { left, top } = await getPopupPosition();
    const popupURL = new URL('../ui/popup.html', import.meta.url);
    popupURL.searchParams.append(params, 'true');
...
export async function openPopup(params) {
    const popup = await createPopup(params);
